...
export default function EquiposGrid({ teams, players }: { teams: Equipo[]; players: Jugador[] }) {
  if (teams.length === 0) {
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-3 sm:gap-4.5 mb-10">
        {ranked.map((t) => (
          <a
            key={t.id}
            href={`#${t.anchor}`}
            className="no-underline bg-white rounded-[10px] overflow-hidden shadow-[0_4px_14px_rgba(0,0,0,0.07)] block"
          >
        ...
        {ranked.map((t) => {
          ...
          return (
            <div key={t.id} id={t.anchor} className="scroll-mt-[90px] bg-white rounded-[10px] shadow-[0_4px_14px_rgba(0,0,0,0.06)] overflow-hidden">
              <div className="flex items-center gap-3 px-5 py-3.5 flex-wrap" style={{ background: t.color || "#0E5C6B" }}>
